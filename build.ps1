...
if ($Export) {
    ...
    $outputPath = Join-Path $PSScriptRoot "bin\SentryKey.prg"
...
if ($Export) {
    Write-Host "Package Succeeded! Output: bin\SentryKey.iq" -ForegroundColor Green
} else {
    Write-Host "Build Succeeded! Output: bin\SentryKey.prg" -ForegroundColor Green
}
...
if ($Run) {
    ...
    $tempPrg = Join-Path $tempDir "SentryKey.prg"
    Copy-Item $outputPath $tempPrg -Force
    
    $settingsSrc = Join-Path $PSScriptRoot "bin\SentryKey-settings.json"
    $appId = "a8d3e91b-cf52-4a87-bb4b-9800d0c85467"
    $settingsNames = @(
        "SentryKey-settings.json",
        "SentryKey.json",
        "$appId.json",
        "$appId-settings.json"
    )
